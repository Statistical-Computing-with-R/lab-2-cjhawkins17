...
The creation of this document referenced ChatGPT.
https://chatgpt.com/share/66ff26ef-a700-8004-a7b3-8f7c8afcefc1
...
# 1
surveys = read_csv("/Users/christopherhawkins/Documents/STAT 531/Week 2/lab-2-cjhawkins17/data/surveys.csv")

# 2
dim(surveys)

# 3
sapply(surveys, class)
```

**2. What are the dimensions (# of rows and columns) of these data?**

Number of rows = 30,463.
...
See output under respective code.
...
ggplot(data = surveys,
  ...
  facet_wrap(~ species) +
  labs(title = "Hindfoot length vs. weight of various rodent species",
...
species_mod <- aov(weight ~ factor(species), data = surveys)
...
Assume a critical value of 0.05, the p-value from the ANOVA test is 2*10^-16 which is less than 0.05. Therefore, the difference in group means is statistically significant. Reject the null hypothesis and support the alternative.
